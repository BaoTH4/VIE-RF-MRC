...
def test(model,batch_generator,standard,logger=None):
  ...
  opi_pol_match_num=0
  opi_pol_predict_num=0
  opi_pol_target_num=0
  ...
  for batch_index,batch_dict in enumerate(batch_generator):
    ...
    sent_labels=[]
    opi_pol_pair_labels=[]
    for opi_pol in standard[batch_index]['opi_pol_target']:
      sent_targets.append([opi_pol[0],opi_pol[-1]])
      sent_labels.append([opi_pol[0],sentiments[0][opi_pol[0]]])
      
    '''for idx in range(len(sentiments[0])):
      if sentiments[0][idx]!=-1:
        sent_labels.append([idx,sentiments[0][idx]])'''

    for opi in opinion_terms[0]:
      opi_pol_pair_labels.append(opi+[sentiments[0][opi[0]]])
    ...
    ##Overall
    opi_pol_predict_num+=len(opi_pol_pair_labels)
    opi_pol_target_num+=len(standard[batch_index]['opi_pol_target'])
    for opi_pol_pred in opi_pol_pair_labels:
      for opi_pol_tar in standard[batch_index]['opi_pol_target']:
        if opi_pol_pred==opi_pol_tar:
          opi_pol_match_num+=1
  ...
  overall_precision = float(opi_pol_match_num) / float(opi_pol_predict_num)
  overall_recall = float(opi_pol_match_num) / float(opi_pol_target_num)
...
def main(args):
  ...
  data_path=f'{args.data_path}/data_bdi_debertav3_xxsmall_new.pt'
  standard_data_path = f"{args.data_path}/data_standard_new.pt"
  ...
  if args.mode=='train':
    args.save_model_path_ae = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_dev_ae_f1' + '.pth'
    args.save_model_path_oe = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_dev_oe_f1' + '.pth'
    args.save_model_path_asc = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_dev_asc_f1' + '.pth'
    args.save_model_path_overall = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_dev_overall_f1' + '.pth'
    args.save_model_path_test_ae = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_test_ae_f1' + '.pth'
    args.save_model_path_test_oe = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_test_oe_f1' + '.pth'
    args.save_model_path_test_asc = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_test_asc_f1' + '.pth'
    args.save_model_path_test_overall = args.save_model_path + '/' + args.model_type[2:] + '/' + args.data_name + '_' + 'best_test_overall_f1' + '.pth'
...
if __name__=='__main__':
    ...
    parser.add_argument('--data_name', type=str, default="MBBank_new", choices=['MBBank','MBBank_new'])

    parser.add_argument('--mode', type=str, default="train", choices=["train", "test"])

    parser.add_argument('--reload', type=bool, default=False)
    parser.add_argument('--checkpoint_path', type=str, default="./model/MBBank_new/modelFinal.model")
    parser.add_argument('--save_model_path', type=str, default="./model_deberta_threshold")
    ...
    parser.add_argument('--entity_size',type=int,default=22)
    ...
    parser.add_argument('--opinion_threshold',type=float,default=0.8)
