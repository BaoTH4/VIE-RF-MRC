...
from utils import is_one_exist, create_entity_to_id
from transformers import DebertaV2Tokenizer
import json


CLS_id=0 ##For Deberta
SEP_id=2 ##For Deberta

entity2id,id2entity=create_entity_to_id(['./data/MBBank/data_sentiment_all.json'])

TOKENIZER=DebertaV2Tokenizer.from_pretrained('./bdi-debertav3-xxsmall')
...
def generating_batch_index(dataset,batch_size,shuffle=True,drop_last=False):
  ...
  '''
  idx_list=[i for i in range(dataset.__len__())] ##Khởi tạo trị số vị trí cho dữ liệu

  if shuffle==True: ##Xáo trộn các vị trí khi shuffle là True
  ...
  #Gom các vị trí thành các batch có độ lớn batch_size
  ...
  ##Lựa chọn có nên lấy batch cuối không theo biết drop_last
...
def generating_one_query(sample,query_type='initial',max_len=None,outside_query=None,index=None,model_mode='train'):
  ...
  if query_type=='initial':
    ...
    ##padding
    input_ids=question+[1]*padding_num
  ...
  ##Xử lý cho dạng query là opinion
  elif query_type=='opinion':

    ##Kiểm tra xem có câu query từ bên ngoài không
    if outside_query==None and index!=None:
      query=sample['opinion_questions_ids'][index]
    else:
      query=outside_query

    ##Trích context và câu trả lời cho aspect
    context=sample['texts_ids'][index]
...
def padding_query_batch(input_ids_list,attention_mask_list,token_type_ids_list,answer_list,query_type='aspect',max_len=None,ifgpu=True):
  '''
  ...
  for i in range(len(input_ids_list)):
    ...
    ##input_ids
    input_ids_list[i].extend([1]*padding_len)
...
def generating_next_query(batch_dict,logits,last_queries,args,query_type='aspect',model_mode='train'):
  '''
    ...
    + Hướng xử lý trường hợp sau khi softmax không sinh ra nhãn 1-(begin opinion)
  ...
  '''
  ##Khởi tạo các danh sách rỗng để chứa các thành phần cần thiết cho input và training
  input_ids_list=[]
  attention_mask_list=[]
  token_type_ids_list=[]
  answer_list=[]

  ##Ghi nhận câu query có độ dài dài nhất để tiến hành padding
  max_len=0
  
  ##Tạo input nếu phần câu hỏi tiếp theo là opinion
  if query_type=='opinion':
    ##Trích xuất ra nhãn của các token
    prob=F.softmax(logits,dim=-1)
    top_val,top_ind=torch.max(prob,dim=-1)
  
    for idx,ind_tensor in enumerate(top_ind):

      ##Trích ra những vị trí trong câu context của query
      ...
      ##Lấy label theo vị trí của văn bản
      labels=ind_tensor[passenge_index].squeeze(1)

      ##Nếu là deberta sẽ có các vị trí cần bỏ qua (sub-word)
      ...
      if is_one_exist(labels,ignore_index)==False:
        ...
        ##Nếu ở chế độ train thì sử dụng teacher forcing
        if model_mode=='train':
          input_ids,attention_mask,token_type_ids,answer=generating_one_query(batch_dict,query_type=query_type,index=idx)

        ##Nếu không phải thì chọn theo độ lớn cho xác suất nhãn 1  
        else:
          outside_query=[] #Biến để ghi nhận query
          ...
          ##Trng trường hợp này ta không cần các vị trí nhãn 2
          two_index=torch.tensor([])
          count=0

          ##Bắt đầu lấy các token id cho query
          for index in one_index:
            inde=index.item()

            ##Nếu vị trí này thuộc các vị trí cần bỏ qua thì bỏ qua
            if inde in ignore_index:
              continue

            ##Tiến hành thêm các token id vào query
            ...
            ##Kiểm tra điều kiện dừng cho việc lấy với hai biến q đại diện cho loại query opinion
            if count>=args.q:
              break

          ##Tạo inputs cho model cho bước tiếp theo  
        ...
        ##Ghi nhận câu có độ dài lớn nhất
        if len(input_ids)>max_len:
          max_len=len(input_ids)

        ##Tiến hành ghi lại các thành phần của input  
      ...
      ##Nếu có nhãn 1 thì lấy top p hoặc q tương ứng với aspect hay query như bình thường
        ...
        outside_query=[] ##Khởi tạo list để ghi nhận query
      
        ##Trích ra các vị trí có nhẵn 1 và 2
        new_top_val=top_val[idx][passenge_index].squeeze(1)
        one_index=(labels == 1).nonzero(as_tuple=True)[0]
        two_index=(labels == 2).nonzero(as_tuple=True)[0]

        ##Sắp xếp các vị trí xác suất theo xác suất giảm dần
        _,_top_ind=torch.sort(new_top_val,descending=True)
        count=0 ##Đếm số lượng aspect hoặc opinion

        ##Bắt đầu trích các aspect hoặc opinion cho query
        for index in _top_ind:
          inde=index.item()

          ##Bỏ qua nếu vị trí đó thuộc hai hoặc vị trí sub-word
          if inde in two_index or inde in ignore_index:
            continue

          ##Tiến hành trích aspect hay opinion khi vị trí là 1
          if inde in one_index:
            outside_query.append(batch_dict['texts_ids'][idx][inde])
            count+=1
            inde+=1

            ##Với các vị trí ngay kế bên vị trí nhãn 1 nếu là sub-word hay vị trí có nhãn hai thì ta cho vào query
            while inde < len(batch_dict['texts_ids'][idx]) and (inde in two_index or inde in ignore_index):
              outside_query.append(batch_dict['texts_ids'][idx][inde])
              inde+=1

            ##Kiểm tra điều kiện dừng theo p hoặc  q
            if count>=args.q:
              break
      
        ##outside_query=outside_query[:-1]
        ##Khởi tạo các thành phần của input query
        input_ids,attention_mask,token_type_ids,answer=generating_one_query(batch_dict,query_type=query_type,index=idx,outside_query=outside_query,model_mode=model_mode)
      
        ##Ghi nhận câu có độ dài lớn nhất để  paddinhg
        if len(input_ids)>max_len:
          max_len=len(input_ids)

        ##Ghi nhận các thành phần của input query
        input_ids_list.append(input_ids)
        attention_mask_list.append(attention_mask)
        token_type_ids_list.append(token_type_ids)
        answer_list.append(answer)
    ...
    for a in range(logits.size(0)):
      
      ##Lấy nhãn và xác suất của nhãn
      logits_a=logits[a]
      F_prob=F.sigmoid(logits_a)
      asp_labels=torch.round(F_prob)

      ##Trường hợp không có nhãn 1 trong aspect label
      if 1 not in asp_labels:

        ##Nếu đang ở trạng thái rain moddel thì teacher forcing
        if model_mode=='train':
          input_ids,attention_mask,token_type_ids,answer=generating_one_query(batch_dict,query_type=query_type,index=a)

        ##Trường hợp không phải training cứ lấy theo xác suất từ trên xuống
        else:
          outside_query='' #Biến để ghi nhận query
          _,indices=torch.sort(F_prob,dim=-1,descending=True)
          count=0 ##Sử dụng làm điều kiện dừng
          for indice in indices:
            b=indice.item()
            outside_query=outside_query+id2entity[b]+' '
            count+=1

            ##Duefng lấy thêm aspect khi đã chạm ngưỡng
            if count>=args.p:
              break

          outside_query=TOKENIZER.encode(outside_query[:-1].lower(),add_special_tokens=False)

          ##Tạo inputs cho model cho bước tiếp theo  
          input_ids,attention_mask,token_type_ids,answer=generating_one_query(batch_dict,query_type=query_type,index=a,outside_query=outside_query,model_mode=model_mode)
      ...
      ##Trường hợp có nhãn 1 trong aspect labels
      else:
        outside_query='' ##Ghi nhận các entity được sử dụng làm query
        one_labels=(asp_labels==1).nonzero(as_tuple=True)[0]
        _,indices=torch.sort(F_prob,dim=-1,descending=True) ##Sắp xếp xác suất theo chiều giảm dần
        count=0 ##Dùng để làm điều kiện duefng
        ...
        for indice in indices:
          ...
          ##Dừng nếu số lượng query lấy được lớn hơn p
          if count>=args.p:
            break
        
        ##Chuyển đỏi query từ dạng chữ sang dạng deberta id
        outside_query=TOKENIZER.encode(outside_query[:-1].lower(),add_special_tokens=False)
  ...
  ##Tiến hành padding    
  inputs_ids,attention_masks,token_type_ids,answers=padding_query_batch(input_ids_list,attention_mask_list,token_type_ids_list,answer_list,query_type=query_type,max_len=max_len,ifgpu=args.ifgpu)
